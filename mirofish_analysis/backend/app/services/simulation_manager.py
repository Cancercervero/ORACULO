...
_listener_logger = logging.getLogger(__name__ + ".listener")


async def listen_and_simulate(simulation_manager=None) -> None:
    """
    Subscribe to council.briefing and publish a sim.result for each briefing.
    Full simulation pipeline requires a prepared Zep graph (available via UI).
    Auto-triggered sims publish a lightweight stub result immediately.
    """
    import datetime
    ...
    while True:
        ...
        try:
            ...
            _listener_logger.info("MiroFish listening on council.briefing")
            ...
            async for message in pubsub.listen():
                ...
                try:
                    ...
                    _listener_logger.info("Auto-launching simulation for incident=%s", incident_id)
                    ...
                    sim_result = {
                        ...
                        "top_narratives": [],
                        "platform_breakdown": {
                            "twitter": {"sentiment": -0.2, "agent_count": 30},
                            "reddit": {"sentiment": -0.2, "agent_count": 20},
                        },
                        "completed_at": datetime.datetime.utcnow().isoformat() + "Z",
                    }
                    await redis_client.publish("sim.result", json.dumps(sim_result))
                    _listener_logger.info("Published sim.result for incident=%s", incident_id)
                except Exception:
                    _listener_logger.exception("Error processing briefing message")
        except asyncio.CancelledError:
            raise
        except Exception:
            _listener_logger.exception("Redis connection lost, reconnecting in 5s")
